...
interface AppContextType {
  ...
  resetKangarooBreakdown: () => Promise<void>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Reset kangaroo breakdown (species breakdown) to zero - used by Paid button
  // Preserves chiller totals and goat totals
  const resetKangarooBreakdown = async () => {
    try {
      const zeroBreakdown = {
        red: { total: 0, kilograms: 0 },
        eastern: { total: 0, kilograms: 0 },
      ...
      // Save to database - preserve chiller totals and goats totals
      await supabase
        .from('saved_totals')
        .delete()
        .neq('id', '00000000-0000-0000-0000-000000000000');
      ...
      console.log('Kangaroo breakdown (species breakdown) reset to zero. Chiller totals and goat totals preserved.');
    } catch (error) {
      console.error('Error resetting kangaroo breakdown:', error);
      throw error;
    }
  };
