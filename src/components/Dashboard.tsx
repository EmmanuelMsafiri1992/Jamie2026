...
const Dashboard: React.FC = () => {
  ...
  const { updateTotalsFromEntries, resetAllTotals, loadStoredTotals, resetKangarooBreakdown } = useAppContext();
  ...
  const paidReset = async () => {
    if (!confirm('Are you sure you want to delete all shooter entries and reset Pays? This will permanently delete all entries from the database, reset species breakdown, and clear all shooter/pay data. Chiller totals and Goat totals will remain unchanged. This action cannot be undone.')) {
      return;
    }

    setIsPaidResetting(true);
    try {
      // Step 1: Delete all entries from the database
      const { error: deleteError } = await supabase
        .from('inventory')
        .delete()
      ...
      // Step 2: Reset kangaroo breakdown (species breakdown) to zero
      // This preserves chiller totals and goat totals
      await resetKangarooBreakdown();
      ...
      alert('All shooter entries have been deleted, species breakdown reset, and Pays cleared! Chiller totals and Goat totals remain unchanged.');
    } catch (error) {
