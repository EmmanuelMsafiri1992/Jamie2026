...
const PaysModal: React.FC<PaysModalProps> = ({ isOpen, onClose, entries }) => {
  const { updateTotalsFromEntries, resetKangarooBreakdown } = useAppContext();
  ...
  const handlePaysReset = async () => {
    if (!confirm('Are you sure you want to reset pays data? This will DELETE ALL shooter entries and pay records, and reset species breakdown. Chiller totals and Goat totals will remain intact.')) {
      return;
    }
    ...
    try {
      ...
      // Reset kangaroo breakdown (species breakdown) to zero
      ...
      alert('All shooter entries and pay data have been deleted, species breakdown reset! Chiller totals and Goat totals remain intact.');
      onClose();
